...
## 待測試（2026-07-03 VU meter 敏銳度）

- [ ] **VU meter 除數再確認**：目前 `_VU_DISPLAY_DIVISOR` 已從 80 → 130 → 650 兩度調鬆（見 CHANGELOG），但都是憑回報數字用比例估算，沒有實測驗證過。需要實際錄音時回報「小聲大概 X%、正常講話大概 Y%」，才能判斷 650 是否洽當或還要再調
