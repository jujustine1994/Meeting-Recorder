...
**多語言**
- 介面支援繁體中文 / 简体中文 / English / 日本語
- 首次啟動跳一次語言選擇視窗，選完存進 `config.json`，之後不再詢問
- 之後要改：進階設定最上方的 `Language:` 下拉選單，選完會問要不要重開套用
- `logs/app.log` 的內容**固定繁中**，不跟介面語言走（維護者要看得懂自己的 log）
