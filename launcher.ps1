# Meeting Recorder 啟動器

[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$host.UI.RawUI.WindowTitle = "Meeting Recorder"
...
#   本程式的錄音套件（pyaudiowpatch）需要 x64 版 Python；uv 依自身執行架構挑
#   對應的 python-build-standalone，CTH 沒有 ARM64 機器可實測，若日後在 ARM64
#   上遇到 pyaudiowpatch 安裝失敗，回來確認是否需要改用 `--python-platform`。
# ======================================
...
if (-not (Test-Path "venv")) {
    ...
    Write-Host "    Meeting Recorder - 首次安裝說明" -ForegroundColor Cyan
    Write-Host "  ============================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "    2. pyaudiowpatch" -ForegroundColor Yellow
    Write-Host "       錄製 Windows 系統輸出音訊（WASAPI Loopback 技術）" -ForegroundColor Gray
    Write-Host ""
    Write-Host "    3. lameenc" -ForegroundColor Yellow
    Write-Host "       將錄音轉換為 MP3 格式（不需要另外安裝 ffmpeg）" -ForegroundColor Gray
    Write-Host ""
    ...
    if ($ans -eq "" -or $ans -ieq "Y") {
        ...
        Write-Host "[INFO] 安裝套件中（首次需要幾分鐘）..." -ForegroundColor Gray
    ...
    # 清理損壞的 dist-info（METADATA 遺失時 uv 會拒絕安裝新套件）
